...
const MessageView = ({ chatId, isSidebarOpen, setIsSidebarOpen }) => {
  ...
  const [sendingMessageLocal, setSendingMessageLocal] = useState(false);
  ...
  // ---------------- Use Subscription only if chatId exists ----------------
...
const handleSubmit = async (e) => {
      ...
      <div className="flex-grow flex items-center dark:bg-black justify-center h-full relative">
        ...
        <div className="text-center max-w-lg">
          <motion.h1
            ...
            className="text-5xl font-bold text-gray-600 dark:text-foreground"
          >
            Meet Convo
          </motion.h1>
          <motion.p
          ...
          >
            Your all-in-one AI companion, built to handle anything.
          </motion.p>
  ...
  if (loading) return <div className="flex-grow flex items-center justify-center h-full"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>;
  if (error) return <div className="flex-grow p-4 text-destructive h-full">Error: {error.message}</div>;
